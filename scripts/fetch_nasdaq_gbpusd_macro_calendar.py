#!/usr/bin/env python3
"""Fetch a cached GBP/USD macro calendar from Nasdaq's public economic calendar.

The output uses the same CSV schema consumed by src.news_guard.calendar_loader.
Each calendar day is cached separately, so reruns reuse prior Nasdaq responses.
"""
...
def is_high_impact(currency: str, event_name: str) -> bool:
    return any(
        re.search(pattern, event_name, flags=re.IGNORECASE)
        for pattern in HIGH_IMPACT_PATTERNS.get(currency, [])
    )
...
def event_id_for(day: date, currency: str, event_name: str, event_time_utc: datetime) -> str:
    ...
    digest = hashlib.sha1(
        f"{event_time_utc.isoformat()}|{currency}|{event_name}".encode()
    ).hexdigest()[:8]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        default="data/macro_calendar/gbp_usd_events_2020_2025_nasdaq.csv",
